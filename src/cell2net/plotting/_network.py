...
def tf_gene_network(
    ...
    if tf_col not in df.columns:
        logger.error(f"Column {tf_col} not found in dataframe")
        return None

    if gene_col not in df.columns:
        logger.error(f"Column {gene_col} not found in dataframe")
        return None

    if activity_col not in df.columns:
        logger.error(f"Column {activity_col} not found in dataframe")
        return None
