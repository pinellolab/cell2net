...
from ._utils import get_mudata_attribute


class BaseMuDataField(ABC):
    """Abstract class for a single MuData field"""
    ...
    @abstractmethod
    def view_state_registry(self, state_registry: dict) -> Table | None:
    ...
    def get_field_data(self, mdata: MuData) -> np.ndarray | pd.DataFrame:
        """Returns the requested data as determined by the field."""
        if self.is_empty:
            raise AssertionError(f"The {self.registry_key} field is empty.")
        return get_mudata_attribute(mdata, self.attr_name, self.attr_key, mod_key=self.mod_key)
...
# Convenience type
MuDataField = type[BaseMuDataField]
